...
@param[in] 16-bits value spevify the desired mode of each pin 1 for Output with max clock
gating[50MHz] 0 for Input floating.

*/
void
portInit(GPIO_TypeDef *GPIO, uint16_t __val)
{
...
@param[in] GPIOx. i.e GPIOA  @ref
@param[in] value wether the value was 1 then it's Output Push-Pull if 0 keep it as it was previously
initialized
*/
void
portInitAlt(GPIO_TypeDef *GPIO, uint16_t _val)
{
	unsigned long __y = 0, __z = 0;
...
*/
void
portWrite(GPIO_TypeDef *GPIO, uint16_t value)
{
	GPIO->ODR = value;
}

void
pinSet(GPIO_TypeDef *GPIO, uint16_t pinvalue)
{
	GPIO->BSRR = (1 << pinvalue);
}

void
pinReset(GPIO_TypeDef *GPIO, uint16_t pinvalue)
{
	GPIO->BRR = (1 << pinvalue);
}
...
*/
uint16_t
portRead(GPIO_TypeDef *GPIO)
{
	return (GPIO->IDR);
}

uint16_t
pinRead(GPIO_TypeDef *GPIO, uint16_t pinvalue)
{
