/* @file 			 : NVIC.c
*  @Description: This file includes the Functions Implementation of the NVIC Register Access and
interrupt Enable per Request. This source file should include functions to do the following
...
*/
void
nvic_clear_pending_irq(u8 irqn)
{
	NVIC_ICPR(irqn / 32) = (1 << (irqn % 32));
}
/** @brief Get the Active Interrupt for the desired peripheral request (check if this peripheral has
   sent IRQ).
        @param[in] interrupt Request which represents IRQ from peripheral
        @example nvic_get_active_irq(NVIC_ADC1_2_IRQ);
*/
u8
nvic_get_active_irq(u8 irqn)
{
...
*/
void
nvic_set_priority(u8 irqn, u8 priority)
{
...
*/
void
nvic_generate_software_interrupt(u8 irqn)
{
	if (irqn <= 239)
		NVIC_STIR |= irqn;
}
