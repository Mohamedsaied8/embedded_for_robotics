...
// double rate,Div;
int __clk = 8;  // Default to 8 MHz if not initialized by initClk()

/*
void usartremap(USART_TypeDef *usart, int setmap){
    if(usart==USART1){
    AFIO->MAPR   &= ~(1<<2);         // reset USART1 remap
        if(setmap==1)   AFIO->MAPR   |= 1<<2;            //   set USART1 remap, PA9 -> PB6 , PA10 ->
PB7
    }
...
usartInit(USART_TypeDef *usart, uint32_t br, int setremap)
{
	if (usart == USART1) {
		...
		if (setremap == 1) {
			RCC->APB2ENR |= 1 << 3;
			RCC->APB2ENR |=
			    0x01;  // enable clock for Alternate Function FOR REMAP// enable clock for GPIOD
			AFIO->MAPR &= ~(1 << 2);
