...
*/
void
timerInit(TIM_GP_TypeDef *TIMER, unsigned int prescaler)
{
	char timer_number;
	if (TIMER == TIM2)
		timer_number = 2;
...
*/
void
advancedTimerInit(TIM_GP_TypeDef *TIMER, unsigned int prescaler, unsigned int source, char dir)
{
	char timer_number;
	if (TIMER == TIM2)
		timer_number = 2;
...
*/
void
initTimerIC(TIM_GP_TypeDef *TIMER, char channel, char edge)
{
	TIMER->CCER &= ~(1 << ((channel - 1) * 4));
	if (edge == RISING)
		TIMER->CCER &= ~(1 << (((channel - 1) * 4) + 1));
	else
		TIMER->CCER |= (1 << (((channel - 1) * 4) + 1));
	if (channel < 3) {
...
*/
void
initTimerOC(TIM_GP_TypeDef *TIMER, unsigned int period, unsigned int duty_cycle, char channel)
{
	TIMER->CCER &= ~(1 << ((channel - 1) * 4));
	TIMER->ARR = period;
	if (channel < 3) {
...
*/
void
initTimerOC4CH(TIM_GP_TypeDef *TIMER, unsigned int period, unsigned int chnnel1_duty_cycle,
               unsigned int chnnel2_duty_cycle, unsigned int chnnel3_duty_cycle,
               unsigned int chnnel4_duty_cycle)
{
	TIMER->CCER &= ~(0xff);
	TIMER->ARR = period;
	TIMER->CCMR1 = 0x6868;
...
unsigned int
readCapture(TIM_GP_TypeDef *TIMER, char channel)
{
	while (!(TIMER->SR & (1 << channel)))
		;                          // wait -
	TIMER->SR &= ~(1 << channel);  // clear flag
	switch (channel) {
		case 1:
			return (TIMER->CCR1);  // capture-compare register
		case 2:
			return (TIMER->CCR2);  // capture-compare register
		case 3:
			return (TIMER->CCR3);  // capture-compare register
		case 4:
			return (TIMER->CCR4);  // capture-compare register
	}
	return 0;
}
/*---------------------------------------------------------------------------*/
...
*/
void
enableTimerInterrupt(TIM_GP_TypeDef *TIMER, char update, char channel)
{
	if (update == 1)
		TIMER->DIER |= 0x01;
	else
...
*/
void
disableTimerInterrupt(TIM_GP_TypeDef *TIMER, char channel)
{
	if (channel != 0)
		TIMER->DIER &= ~(1 << channel);
	if (TIMER == TIM2)
